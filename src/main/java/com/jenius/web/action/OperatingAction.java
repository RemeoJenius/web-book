...
public class OperatingAction extends ActionSupport implements ModelDriven<Product> {
	...
	public String buyProduct() {
		...
		if (request.getParameter("id") != null)// 遇到情况 购买商品 要获取商品id 在主页面使用http
												// get方法所以用的request获取id
												// 而详情页要用session获取id所以要判断
		{
	...
	public String deleteProduct()// 卖家删除商品
	{
		...
		GetProductInfo getProductInfo = context.getBean("getProductInfo", GetProductInfo.class);// 获取
																								// mybatis
																								// 接口对象
		if (DeleteImage.deleteFile(getProductInfo.getProductsInfoById(id).getImageAdress()))// 删除商品图片
																							// 方法的参数是图片的地址
		{
			this.result.put("success", true);
			this.result.put("message", "购买成功");
	...
	public String editData()// 卖家点击修改 回到一个表单页，表单中有值 ，值是从数据库中取出来填充到容器中
	{
		...
		session.put("product", getProductInfo.getProductsInfoById(id));// 用mybatis
																		// 接口方法将数据库中查到的商品加到容器中
		return "editData";
	}
